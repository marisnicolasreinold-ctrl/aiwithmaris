// Port von scripts/og/render-ebook.mjs für Deno-Edge-Functions: kein fs,
// Fonts als Buffer (per fetch geladen), Ausgabe als Uint8Array.
// Layout-Logik 1:1 übernommen — bei Änderungen IMMER beide Dateien pflegen.
...
type Block = { t: string; text?: string; items?: string[]; rows?: string[][] };

function parse(raw: string) {
  let src = raw.replace(/\r\n/g, "\n");
  const meta: Record<string, string> = { title: "", subtitle: "", author: "Maris Reinold", lang: "de" };
  const fm = src.match(/^---\n([\s\S]*?)\n---\n/);
  if (fm) {
...
export async function renderPdf(raw: string, fonts: Record<string, Uint8Array>): Promise<{ bytes: Uint8Array; pages: number }> {
  ...
  for (const [k, b] of Object.entries(fonts)) doc.registerFont(k, Buffer.from(b));
  ...
  if (meta.kicker) L.cover = meta.kicker;

  function richText(text: string, opts: Record<string, unknown> = {}) {
    const segs = text.split(/(\*\*[^*]+\*\*)/).filter(Boolean);
    const baseFont = (opts.font as string) || "body";
    const size = (opts.size as number) || 10.5;
    const color = (opts.color as string) || INK;
    segs.forEach((seg, idx) => {
      const isBold = /^\*\*.*\*\*$/.test(seg);
      const t = isBold ? seg.slice(2, -2) : seg;
      doc.font(isBold ? "bold" : baseFont).fontSize(size).fillColor(isBold ? ((opts.boldColor as string) || color) : color)
        .text(t, (opts.x as number) ?? undefined, idx === 0 ? (opts.y as number) ?? undefined : undefined, {
          width: (opts.width as number) || CW, align: (opts.align as string) || "left", lineGap: (opts.lineGap as number) ?? 3.4,
          continued: idx < segs.length - 1, indent: 0, features: [],
        });
    });
  }
  function richHeight(text: string, { size = 10.5, width = CW, lineGap = 3.4, font = "body" } = {}) {
  ...
  doc.font("body").fontSize(14).fillColor("#c9cfdd").text(meta.subtitle, M.left, doc.y + 40, { width: CW - 60, lineGap: 4, features: [] });
  ...
  for (const b of blocks) {
    if (b.t === "h1") {
      chapterNo++;
      doc.addPage();
      doc.font("monobold").fontSize(9.5).fillColor(CYAN).text(`${L.chapter} ${String(chapterNo).padStart(2, "0")}`, M.left, M.top, { characterSpacing: 3 });
      doc.moveDown(0.5);
  ...
  doc.font("body").fontSize(11.5).fillColor("#aab0c4").text(
    meta.lang === "en" ? "More guides, the daily blog and a free intro call:" : "Mehr Leitfäden, der tägliche Blog und ein kostenloses Erstgespräch:",
  ...
  return { bytes: out, pages: range.count };
}
